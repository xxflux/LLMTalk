import { CitationProviderContext, CodeBlock, LinkPreviewPopover } from '@/common/components';
...
export const mdxComponents: ComponentProps<typeof MDXRemote>['components'] = {
    Source: ({ children }) => {
        ...
        return (
            <LinkPreviewPopover source={source}>
                <div className="bg-surface-hover text-surface-hover-foreground/50 hover:bg-primary group mx-0.5 inline-flex size-3.5 flex-row items-center justify-center gap-1 rounded-sm text-[10px] font-medium hover:text-white">
                    {source?.index}
                </div>
            </LinkPreviewPopover>
        );
    ...
    code: ({ children, className }) => {
        if (!className) {
            return (
                <code className="border-primary/20 !bg-primary/10 text-primary rounded-md border px-1.5 py-0.5 font-mono text-sm">
                    {children}
                </code>
            );
        }
