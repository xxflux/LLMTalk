import { ChatEditor, markdownStyles } from '@/common/components';
import { useAgentStream, useChatEditor, useCopyText } from '@/common/hooks';
import { useChatStore } from '@/common/store';
...
import { ImageMessage } from './thread-image-message';
type MessageProps = {
    message: string;
...
export const Message = memo(({ message, imageAttachment, threadItem }: MessageProps) => {
    ...
    return (
        <div className="flex w-full flex-col items-end gap-2 pt-4">
            ...
            <div
                className={cn(
                    'text-foreground bg-surface group relative max-w-[80%] overflow-hidden rounded-lg',
            ...
            >
                {!isEditing && (
                    <>
                        ...
                        >
                            <div className="via-surface/85 to-surface flex w-full items-center justify-end gap-1 bg-gradient-to-b from-transparent p-1.5">
                                {showExpandButton && (
                                    <Button
                                        variant="secondary"
                                        size="xs"
