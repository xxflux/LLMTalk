'use client';
import { DotSpinner } from '@/common/components';
import { useApiKeysStore, useChatStore } from '@/common/store';
import { ChatMode, ChatModeConfig } from '@/shared/config';
import {
    Button,
    cn,
    DropdownMenu,
...
import { BYOKIcon } from '../icons-component';
...
// Combine all models
export const modelOptions = [...openAIModels, ...anthropicModels, ...geminiModels];
...
export const WebSearchButton = () => {
    ...
    useEffect(() => {
        hasApiKeyForChatMode(chatMode).then(setHasApiKey);
    }, [chatMode, hasApiKeyForChatMode]);

    if (!ChatModeConfig[chatMode]?.webSearch && !hasApiKey) return null;
    ...
    useEffect(() => {
        const checkApiKeys = async () => {
            ...
            for (const option of modelOptions) {
                status[option.value] = await hasApiKeyForChatMode(option.value);
            }
                ...
                {openAIModels.map(option => (
                    ...
                    >
                        <div className="flex w-full flex-row items-center gap-2.5 px-1.5 py-1.5">
                            ...
                            <div className="flex-1" />
                            {apiKeyStatus[option.value] && <BYOKIcon />}
                        </div>
                    </DropdownMenuItem>
                ))}
            </DropdownMenuGroup>
