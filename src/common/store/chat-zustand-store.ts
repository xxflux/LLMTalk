...
import { Model, models } from '@/common/lib/ai/models';
import { ChatMode } from '@/shared/config';
import { MessageGroup, Thread, ThreadItem } from '@/shared/types';
...
const initializeWorker = () => {
    console.log('[DEBUG] initializeWorker called');
    
    if (typeof window === 'undefined') {
        console.log('[DEBUG] Window is undefined, skipping worker init');
        return;
    }

    try {
        console.log('[DEBUG] Attempting to create SharedWorker');
        console.log('[DEBUG] import.meta.url:', import.meta?.url);
        
        const workerUrl = new URL('./db-sync-web-worker.ts', import.meta?.url);
        console.log('[DEBUG] Worker URL:', workerUrl.href);
        ...
        console.log('[DEBUG] SharedWorker created successfully');
        ...
        dbWorker.onerror = err => {
            console.error('[DEBUG] SharedWorker error:', err);
...
const initializeTabSync = () => {
    console.log('[DEBUG] initializeTabSync called (fallback method)');
    
    if (typeof window === 'undefined') {
        console.log('[DEBUG] Window is undefined in tabSync');
        return;
    }
...
export const useChatStore = create(
    immer<State & Actions>((set, get) => ({
        ...
        updateThreadItem: async (threadId, threadItem) => {
            if (!threadItem.id) {
                console.warn('[DEBUG STORE] updateThreadItem called without id');
                return;
            }
            if (!threadId) {
                console.warn('[DEBUG STORE] updateThreadItem called without threadId');
                return;
            }

            try {
                console.log('[DEBUG STORE] updateThreadItem called:', {
                    threadId,
                ...
                console.log('[DEBUG STORE] Existing item found:', !!existingItem);
                ...
                set(state => {
                    ...
                    if (index !== -1) {
                        console.log('[DEBUG STORE] Updating existing item at index:', index);
                        state.threadItems[index] = updatedItem;
                    } else {
                        console.log('[DEBUG STORE] Adding new item to threadItems');
...
if (typeof window !== 'undefined') {
    ...
    loadInitialData().then(
        ({
            ...
            console.log('[DEBUG] Checking SharedWorker support');
            console.log('[DEBUG] SharedWorker in window:', 'SharedWorker' in window);
            
            if ('SharedWorker' in window) {
                console.log('[DEBUG] SharedWorker is supported, initializing...');
                initializeWorker();
            } else {
                console.log('[DEBUG] SharedWorker NOT supported, using localStorage fallback');
